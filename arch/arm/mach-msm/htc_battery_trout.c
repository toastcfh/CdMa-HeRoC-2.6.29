...
#define GPIO_MCHG_EN		22		
#define GPIO_ISET			16		

#if !defined(CONFIG_MACH_FIRESTONE) && !defined(CONFIG_MACH_MAHIMAHI) && !defined(CONFIG_MACH_INCREDIBLE)
...
static int htc_is_cable_in(void)
{
	if (!htc_batt_info.update_time) {
		BATT("%s: battery driver hasn't been initialized yet.\n", __func__);
		return -EINVAL;
	}
...
//EXPORT_SYMBOL(htc_power_policy);
/*
 * Jay, 7/1/09'
 */
...
#if 1 /*TODO: Wait battery driver ready*/
	if (!machine_is_orca() && !machine_is_incredible())
#endif	
				    ...
				    union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_CAPACITY:
		...
		/* Work arround for dummy battery 0% shutdown. */
		if (htc_batt_info.rep.level == 0)
			val->intval = 15;
		/* Work arround for Zygote restart when battery < 15% */
		else if (htc_batt_info.rep.level < 15 && htc_batt_info.rep.level >= 4)
			val->intval = 15;
		/* Prevent low battery not shutdown. */
		else if (htc_batt_info.rep.level < 4)
			val->intval = 0;
		else
			val->intval = htc_batt_info.rep.level;

		/* TODO: remove it after battery driver ready. */
		if (machine_is_incredible() || machine_is_incrediblec())
			 val->intval = 15; 	
#else
		val->intval = htc_batt_info.rep.level;
#endif
