...
unsigned int heroc_get_hwid(void)
{
	return hwid;
}

unsigned int heroc_get_skuid(void)
{
	return skuid;
}

unsigned heroc_get_engineerid(void)
{
	return engineerid;
}
...
/*static int touch_reset(void)
{
	...
	gpio_set_value(HEROC_GPIO_TP_EN, 1);
	msleep(260);
	gpio_set_value(HEROC_GPIO_WAKE_UP, 1);
	msleep(40);
	gpio_set_value(HEROC_GPIO_WAKE_UP, 0);
...
/*static struct melfas_i2c_rmi_platform_data heroc_melfas_ts_data[] = {
	{
		.version = MELFAS_DIAMOND_PATTERN,
		.wake_up = HEROC_GPIO_WAKE_UP,
		.intr = HEROC_GPIO_TP_ATT_N,
		.reset = touch_reset,
		.tp_en = HEROC_GPIO_TP_EN,
	},
	{
...
static void __init heroc_init(void)
{
	...
	//msm_change_usb_id(0x0bb4, 0x0c07);
	...
	if (system_rev > 1)
		heroc_h2w_data.flags = REVERSE_MIC_SEL | _35MM_MIC_DET_L2H;
