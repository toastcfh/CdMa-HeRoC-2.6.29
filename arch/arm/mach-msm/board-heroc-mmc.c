...
static int heroc_wifi_cd = 0;		/* WIFI virtual 'card detect' status */
...
static int heroc_wifi_status_register(void (*callback)(int card_present, void *dev_id), void *dev_id)
{
	if (wifi_status_cb)
		return -EAGAIN;
...
int __init heroc_init_mmc(unsigned int sys_rev)
{
	...
	if (!opt_disable_sdcard)
		 msm_add_sdcc(2, &heroc_sdslot_data,
	        	     MSM_GPIO_TO_INT(HEROC_GPIO_SDMC_CD_N), IORESOURCE_IRQ_LOWEDGE | IORESOURCE_IRQ_HIGHEDGE); /* r porting 29 */
