...
static const unsigned short heroc_keymap_0[ARRAY_SIZE(heroc_col_gpios) * ARRAY_SIZE(heroc_row_gpios)] = {
...
uint16_t heroc_axis_map(struct gpio_event_axis_info *info, uint16_t in)
{
...
static struct gpio_event_platform_data heroc_keypad_data = {
	.names = {
		"heroc-keypad",
		"heroc-nav",
		NULL,
...
static struct platform_device heroc_keypad_device = {
	...
	.id = 0,
	.dev = {
		.platform_data = &heroc_keypad_data,
	},
};
...
static int __init heroc_init_keypad(void)
{
	if (!machine_is_heroc())
		return 0;
	if(system_rev == 0)
		heroc_keypad_matrix_info.keymap = heroc_keymap_0;
	else if(system_rev == 1)
		heroc_keypad_matrix_info.keymap = heroc_keymap;
