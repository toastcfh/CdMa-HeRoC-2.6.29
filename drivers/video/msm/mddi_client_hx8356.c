...
static void
himax_sendout_region(struct panel_info *panel)
{
...
static void
himax_update_framedata(struct panel_info *panel)
{
...
static inline void himax_clear_vsync(void)
{
	unsigned val;
	int retry = 1000;
...
static int himax_wait_vsync(void *data)
{
	...
	while (!kthread_should_stop()) {
		rc = wait_event_interruptible(himax_vsync_wait,
				panel->himax_got_int &&
				panel->fb_callback);
		if (rc != 0)
			continue;
		...
		if (panel->vsync_counter <= 0)
			himax_sendout_region(panel);
		else
			himax_update_framedata(panel);
	}
	do_exit(0);
}
...
static int mddi_himax_probe(struct platform_device *pdev)
{
	...
	kthread_run(himax_wait_vsync, panel, "khimax");
	return 0;
